...
function ChartTooltip({ active, payload, label }: { active?: boolean; payload?: { value: number; name: string }[]; label?: string }) {
  ...
  return (
    <div className="rounded-md border border-border bg-bg-panel px-2.5 py-1.5 text-[12px] shadow-md">
      <div className="mb-1 font-medium text-text-primary">KW {label}</div>
      {payload.map((p, i) => (
...
export function WeeklyChart({ weeks }: { weeks: WeekPoint[] }) {
  const data = weeks.map((w) => ({ ...w, wk: w.weekNumber }));
  return (
    <ResponsiveContainer width="100%" height={200}>
      <ComposedChart data={data} margin={{ top: 8, right: 8, left: -16, bottom: 0 }} barCategoryGap={18}>
        <XAxis
          dataKey="wk"
        ...
        <Bar dataKey="nettoHours" name="Ist" fill="var(--accent)" radius={[3, 3, 0, 0]} maxBarSize={22} />
        <Line
          dataKey="sollHours"
          name="Soll"
